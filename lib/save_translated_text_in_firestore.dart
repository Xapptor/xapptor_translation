...
save_translated_text_in_firestore({
  ...
  if (translated_text_query.docs.isNotEmpty) {
    ...
    var first_doc_data_translations = first_doc_data["translations"] as Map<String, dynamic>;

    first_doc_data_translations[target_language] = value;

    await XapptorDB.instance.collection("translations").doc(first_doc.id).update({
      "last_time_used": Timestamp.now(),
      "translations": first_doc_data_translations,
    });
  } else {
    await XapptorDB.instance.collection("translations").doc().set({
      "last_time_used": Timestamp.now(),
